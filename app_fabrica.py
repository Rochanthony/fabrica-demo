...
st.set_page_config(page_title="SaaS Fabrica 5.4", layout="wide")
        ...
        if produto_selecionado:
            ...
            if not df_receita.empty:
                ...
                for index, row in df_receita.iterrows():
                    ...
                    # Input numérico protegido
                    val = c2.number_input(
                        f"Real ({ingrediente})", 
                        value=qtd_meta, 
                        step=0.1, 
                        key=f"in_{ingrediente}_{produto_selecionado}"
                    )
                ...
                # AQUI ESTAVA O ERRO DE SINTAXE ANTES - CORRIGIDO:
                ...
                if st.button("💾 FINALIZAR ORDEM E GERAR PDF", type="primary"):
                    ...
                    if ok:
                        ...
                        try:
                            ...
                            st.error(f"Erro ao gerar PDF: {e_pdf}. Verifique se o FPDF está instalado.")
                    else:
                        st.error(msg)
            else:
                st.warning("Produto sem receita cadastrada.")
        else:
            st.info("Cadastre produtos na aba Cadastros primeiro.")

# --- ABA 2: ESTOQUE (COM BARRAS HORIZONTAIS) ---
with aba_estoque:
...
# --- ABA 4: CADASTROS (EDIÇÃO HABILITADA) ---
with aba_cadastros:
    ...
    with col_mat:
        ...
        with tab_edit_mat:
            ...
            if not df_mats.empty:
                # Selectbox só aceita lista de strings
