...
st.set_page_config(page_title="SaaS TeCHemical v9.2 - Safe Mode", layout="wide")
...
def gerar_pdf_lote(data, operador, produto, itens_realizados, unidades_dict, codigos_dict, custo_plan, custo_real, diferenca):
    ...
    if diferenca >= 0:
        pdf.set_text_color(0, 128, 0); status = f"ECONOMIA: R$ {diferenca:.2f}"
    else:
        pdf.set_text_color(255, 0, 0); status = f"PREJUIZO: R$ {diferenca:.2f}"
...
with st.sidebar:
    st.header("🏭 Painel")
    if st.button("🔴 DELETAR BANCO TRAVADO"):
        try:
    ...
    with col_simulacao:
        if produto_selecionado:
            ...
            if not df_receita.empty:
                consumo_real = {}; unidades_dict = {}; codigos_dict = {}
                custo_planejado = 0.0; custo_real = 0.0
                for index, row in df_receita.iterrows():
                    ingrediente = row['ingrediente']; qtd_meta = float(row['qtd_teorica'])
                    custo_unit = float(row['custo']); unidade_mat = str(row['unidade'])
                    unidades_dict[ingrediente] = unidade_mat; codigos_dict[ingrediente] = str(row['codigo'])
                    ...
                    custo_real += (val * custo_unit); consumo_real[ingrediente] = val
                st.divider()
...
with aba_estoque:
    ...
    if not df_estoque.empty:
        ...
        for i, row in df_estoque.iterrows():
            col_atual = cols[i % 3]
            nome = str(row['nome']); est_atual = float(row['estoque'])
            est_min = float(row['estoque_minimo']); unidade = str(row['unidade'])
            ...
            with col_atual:
                st.markdown(f"""
                <div style="border: 1px solid #444; padding: 10px; border-radius: 8px; margin-bottom: 10px; background-color: #262730;">
                    <b>{nome}</b><br>
                    <span style="color: {cor}; font-size: 1.2em;">{est_atual:.0f} {unidade}</span> <small>(Min: {est_min:.0f})</small>
                    <div style="width: 100%; background: #444; height: 8px; border-radius: 4px; margin-top: 5px;">
                        <div style="width: {pct}%; background: {cor}; height: 8px; border-radius: 4px;"></div>
                    </div>
                    <small style="color: {cor}">{status}</small>
                </div>
...
with aba_cadastros:
    ...
    with c1:
        ...
        with st.form("mat"):
            n = st.text_input("Nome"); cd = st.text_input("Código")
            c = st.number_input("Custo"); e = st.number_input("Estoque")
            u = st.selectbox("Unid", ["kg", "L", "un"]); em = st.number_input("Estoque Mínimo")
            if st.form_submit_button("Salvar") and n:
                cadastrar_material(n, c, e, u, cd, em); st.rerun()
    with c2:
        st.subheader("Produtos")
        prod = st.text_input("Novo Produto")
        if st.button("Criar") and prod: save_prod_code(prod, "PA-NEW"); st.rerun()
