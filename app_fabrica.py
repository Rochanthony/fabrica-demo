...
def popular_dados_iniciais():
    ...
    try:
        ...
        if c.fetchone()[0] == 0:
            # Materiais com CÓDIGO (MP-XXX)
            materiais = [
                ('Resina Epóxi', 15.0, 1000.0, 'kg', 'MP-101'), 
                ('Solvente X', 8.5, 800.0, 'L', 'MP-102'), 
                ('Pigmento Azul', 25.0, 200.0, 'kg', 'MP-103'), 
                ('Aditivo Secante', 45.0, 100.0, 'L', 'MP-104'),
                ('Lata 18L', 12.0, 500.0, 'un', 'EM-001')
            ]
...
def get_materiais_db():
    ...
    try:
        ...
        df = pd.DataFrame(columns=['nome', 'custo', 'estoque', 'unidade', 'codigo'])
    finally: conn.close()
    return df
...
# --- CADASTRO ---
def cadastrar_material(nome, custo, estoque, unidade, codigo):
    conn = sqlite3.connect('fabrica.db')
    try:
        conn.execute("INSERT INTO materiais VALUES (?, ?, ?, ?, ?)", (str(nome), float(custo), float(estoque), str(unidade), str(codigo)))
...
with aba_estoque:
    ...
    if not df_estoque.empty:
        # Mostra tabela primeiro para ver os códigos
        st.dataframe(df_estoque[['codigo', 'nome', 'estoque', 'unidade', 'custo']], use_container_width=True, hide_index=True)
        st.divider()
        st.subheader("Visual")
        cols = st.columns(3)
        for i, row in df_estoque.iterrows():
            with cols[i % 3]:
                st.info(f"**{row['nome']}**\n\nCod: {row['codigo']}\n\n## {row['estoque']} {row['unidade']}")
...
with aba_cadastros:
    ...
    with c1:
        ...
        with st.form("mat_form"):
            ...
            est = col_b.number_input("Estoque", min_value=0.0)
            uni = col_c.selectbox("Unid.", ["kg", "L", "un", "m"])
            if st.form_submit_button("Salvar Material"):
                if nome and codigo:
                    ok, m = cadastrar_material(nome, custo, est, uni, codigo)
    ...
    with c2:
        ...
        if tipo == "Criar Novo Produto":
            ...
            if prod_novo:
                save_prod_code(prod_novo, cod_novo) # Salva o código
